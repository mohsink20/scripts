...
void run_dns_enum(const char *domain)
{
    ...
    if (output_file)
    {
        fclose(output_file);
        char command[256];
        snprintf(command, sizeof(command), "%s >> dns_enum_log.txt 2>&1", dns_enum_command);
        system(command);
    }
    else
    {
...
void run_banner_grabbing(const char *ip_address, int port)
{
    ...
    if (output_file)
    {
        fclose(output_file);
        char command[256];
        snprintf(command, sizeof(command), "%s >> banner_grabbing_log.txt 2>&1", banner_grab_command);
        system(command);
    }
    else
    {
